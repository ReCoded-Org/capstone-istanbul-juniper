...
const Cards = () => {
    ...
    useEffect(() => {
        matchedCardIndexes.forEach(matchedIndex => {
            const card = refArr[matchedIndex].current;
            const oldChild = refArr[matchedIndex].current.children[1];
            const newChild = refArr[matchedIndex].current.children[0];
            newChild.style.display = 'inline-block';
            card.replaceChild(newChild, oldChild);
        });
    }, [matchedCardIndexes]);
